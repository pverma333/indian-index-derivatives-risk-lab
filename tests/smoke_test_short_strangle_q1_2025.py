...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Smoke test: ShortStrangleStrategy on Q1 2025 curated CSV")
    ...
    # Strategy knobs
    p.add_argument("--otm_distance_points", type=float, default=None, help="Override OTM distance; if omitted uses tenor defaults (300/400).")
    ...
    p.add_argument("--save-out", type=str, default="outputs/short_strangle_trades_Q1_2025.csv")
    return p.parse_args()
...
def _default_dist_for_tenor(tenor: str) -> float:
    return 300.0 if tenor == "WEEKLY" else 400.0


def _base_cfg(args: argparse.Namespace, tenor: str) -> dict:
    strike_band_n = 10 if tenor == "WEEKLY" else 15
    dist = float(args.otm_distance_points) if args.otm_distance_points is not None else _default_dist_for_tenor(tenor)

    return {
        "symbol": args.symbol,
        "tenor": tenor,
        "qty_lots": int(args.qty_lots),
        "otm_distance_points": dist,
...
def main() -> None:
    _setup_logging()
    log = logging.getLogger("smoke_short_strangle_q1_2025")
    ...
    for tenor in _tenors(args.tenor):
        ...
        print(
            ...
            f"otm_distance_points={cfg['otm_distance_points']} ===================="
        )
        ...
        if n_dbg > 0:
            ...
            for _, r in sample.iterrows():
                ...
                print(f"cycle dbg: expiry_dt={expiry_dt.date()} entry_date={entry_date.date()} chain_rows={len(chain_df)}")
        ...
        if not trades_df.empty:
            ...
            if not set(trades_df["side"].unique()).issubset({-1}):
                print("WARNING: unexpected side values:", sorted(trades_df["side"].unique().tolist()))
            if set(trades_df["option_typ"].unique().tolist()) != {"CE", "PE"}:
                print("WARNING: unexpected option_typ values:", sorted(trades_df["option_typ"].unique().tolist()))
    ...
    if combined.empty:
        print(
            ...
            "- NO_VALID_ATM_STRIKE => filtered chain cannot support ATM selection\n"
            "- NO_OTM_CE_ABOVE / NO_OTM_PE_BELOW => strike availability issue after filtering\n"
        )

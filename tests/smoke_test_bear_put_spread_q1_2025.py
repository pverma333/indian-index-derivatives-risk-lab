...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Smoke test: BearPutSpreadStrategy on Q1 2025 curated CSV")
    ...
    p.add_argument("--save-out", type=str, default="outputs/bear_put_spread_trades_Q1_2025.csv")
    return p.parse_args()
...
def main() -> None:
    _setup_logging()
    log = logging.getLogger("smoke_bear_put_spread_q1_2025")
    ...
    for tenor in _tenors(args.tenor):
        ...
        print(
            ...
            f"width_points={cfg['width_points']} ===================="
        )
        ...
        if not trades_df.empty:
            ...
            if not set(trades_df["option_typ"].unique()).issubset({"PE"}):
    ...
    if combined.empty:
        print(
            ...
            "- NO_OTM_PUT_BELOW => strike availability issue after filtering\n"
        )

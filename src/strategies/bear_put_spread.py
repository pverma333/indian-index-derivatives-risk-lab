...
#   python src/strategies/bear_put_spread.py
...
class BearPutSpreadStrategy:
    ...
    def build_trades(self, market_df: pd.DataFrame, cfg: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
        ...
        if cycles_df.empty:
            logger.info(
                "No cycles produced for bear put spread.",
        ...
        for _, cyc in cycles_df.iterrows():
            ...
            otm_put_strike = self._select_otm_put_strike_below(
                filt_df=filt_df,
                atm_strike=atm_strike_f,
                width_points=width_points,
            )
            if otm_put_strike is None:
                self._log_skip(_SkipContext(self.strategy_name, symbol, tenor, expiry_dt, entry_date, "NO_OTM_PUT_BELOW"))
                continue

            if not self._has_pe_at_strike(filt_df, atm_strike_f):
                self._log_skip(_SkipContext(self.strategy_name, symbol, tenor, expiry_dt, entry_date, "ATM_PE_MISSING"))
                continue
            if not self._has_pe_at_strike(filt_df, float(otm_put_strike)):
                self._log_skip(_SkipContext(self.strategy_name, symbol, tenor, expiry_dt, entry_date, "OTM_PE_MISSING"))
                continue
            ...
            rows.append(
                self._build_leg_row(
                    ...
                    entry_date=entry_date,
                    exit_date=exit_date,
                    strike_pr=float(otm_put_strike),
                    option_typ="PE",
                    side=-1,
                    strike_interval_used=strike_interval_used,
                )
    ...
    def _make_trade_id(symbol: str, tenor: str, expiry_dt: pd.Timestamp, entry_date: pd.Timestamp) -> str:
        return f"bear_put_spread|{symbol}|{tenor}|{expiry_dt:%Y%m%d}|{entry_date:%Y%m%d}"

    @staticmethod
    def _log_skip(ctx: _SkipContext) -> None:
        logger.info("Skipping bear put spread cycle.", extra=ctx.to_log_dict())

    @staticmethod
    def _has_pe_at_strike(df: pd.DataFrame, strike: float) -> bool:
        return ((df["option_typ"] == "PE") & (df["strike_pr"] == strike)).any()

    @staticmethod
    def _select_otm_put_strike_below(filt_df: pd.DataFrame, atm_strike: float, width_points: float) -> Optional[float]:
        """
        Preferred OTM put strike = ATM - width_points.
        If exact strike is missing: choose nearest available PE strike below ATM.
        Tie-break: choose the larger strike (closer to ATM) deterministically.
        """
        ...
        pe_sorted = sorted(float(s) for s in pe_strikes)

        if preferred in pe_sorted:
            return float(preferred)

        # nearest to preferred; tie -> larger strike (closer to ATM)
        best = min(pe_sorted, key=lambda s: (abs(s - preferred), -s))
        return float(best)
    ...
    def _validate_cfg(cfg: Dict[str, Any]) -> None:
        ...
        if missing:
            raise ValueError(f"BearPutSpreadStrategy cfg missing required keys: {missing}")

        tenor = str(cfg["tenor"])
        if tenor not in {"WEEKLY", "MONTHLY"}:
            raise ValueError(f"BearPutSpreadStrategy expects single tenor per call, got tenor={tenor!r}")
...
if __name__ == "__main__":
    ...
    print("bear_put_spread.py imported OK.")
